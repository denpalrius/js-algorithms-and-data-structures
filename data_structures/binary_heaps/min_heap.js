import { BinaryHeap } from "./binary_heap.js";

class MinHeap extends BinaryHeap {
  constructor() {
    super();
  }

  /**
   * Inserts a new element into the heap and maintains min-heap property.
   * @param {number} element - The element to be inserted.
   */
  insert(element) {
    this.values.push(element);
    this.bubbleUp();
  }

  /**
   * Ensures the last inserted element is placed correctly to maintain heap order.
   */
  bubbleUp() {
    let idx = this.values.length - 1;
    const elementVal = this.values[idx];

    while (idx > 0) {
      const parentIdx = Math.floor((idx - 1) / 2);
      const parent = this.values[parentIdx];

      if (elementVal >= parent) break; // Stop if heap property is maintained

      // Swap the current element with its parent
      this.values[idx] = parent;
      this.values[parentIdx] = elementVal;

      // Move up the heap
      idx = parentIdx;
    }
  }

  /**
   * Removes and returns the minimum element (root of the heap).
   * @returns {number|null} The minimum element or null if the heap is empty.
   */
  extractMin() {
    if (this.values.length === 0) return null; // Empty heap
    if (this.values.length === 1) return this.values.pop(); // Only one element case

    const min = this.values[0]; // Root is the min element
    this.values[0] = this.values.pop(); // Remove last element and move it to the root

    this.sinkDown(); // Restore heap order

    return min;
  }

  /**
   * Ensures the root element is placed correctly after extraction to maintain heap order.
   */
  sinkDown() {
    const length = this.values.length;
    let idx = 0;
    const element = this.values[idx];

    while (true) {
      const leftChildIdx = 2 * idx + 1;
      const rightChildIdx = 2 * idx + 2;

      let leftChild, rightChild;
      let swap = null; // Track index to swap with

      // Check left child
      if (leftChildIdx < length) {
        leftChild = this.values[leftChildIdx];
        if (leftChild < element) {
          swap = leftChildIdx;
        }
      }

      // Check right child
      if (rightChildIdx < length) {
        rightChild = this.values[rightChildIdx];
        if (
          (swap === null && rightChild < element) ||
          (swap !== null && rightChild < leftChild)
        ) {
          swap = rightChildIdx;
        }
      }

      if (swap === null) break; // Stop if heap order is maintained

      // Swap the element with the smaller child
      this.values[idx] = this.values[swap];
      this.values[swap] = element;

      // Move down the heap
      idx = swap;
    }
  }
}

// Run tests
console.log("==== Running MinHeap Tests ====");

function test(actual, expected, testName) {
  const res =
    JSON.stringify(actual) === JSON.stringify(expected)
      ? "✅ passed"
      : "🆘 failed";
  console.log(
    `${testName}: ${res} (got ${JSON.stringify(
      actual
    )}, expected ${JSON.stringify(expected)})`
  );
}

function isMinHeap(arr) {
  for (let i = 0; i < arr.length; i++) {
    const leftChildIdx = 2 * i + 1;
    const rightChildIdx = 2 * i + 2;

    if (leftChildIdx < arr.length && arr[leftChildIdx] < arr[i]) {
      return false; // Left child violates min-heap property
    }

    if (rightChildIdx < arr.length && arr[rightChildIdx] < arr[i]) {
      return false; // Right child violates min-heap property
    }
  }
  return true;
}

const heap = new MinHeap();

// Test Initial Insertion
heap.insert(41);
heap.insert(39);
heap.insert(33);
heap.insert(18);
heap.insert(27);
heap.insert(12);
test(
  isMinHeap(heap.values),
  true,
  "Heap after initial insertions (min-heap property)"
);

// Test Inserting 5 (should bubble up to the root)
heap.insert(5);
test(
  isMinHeap(heap.values),
  true,
  "Heap after inserting 5 (min-heap property)"
);

// Test Extracting Min (should remove 5)
const removed1 = heap.extractMin();
test(removed1, 5, "Extracted min element (should be 5)");
test(
  isMinHeap(heap.values),
  true,
  "Heap after removing min (min-heap property)"
);

// Test Extracting Min Again (should remove 12)
const removed2 = heap.extractMin();
test(removed2, 12, "Extracted min element (should be 12)");
test(
  isMinHeap(heap.values),
  true,
  "Heap after removing second min (min-heap property)"
);

// Edge Case: Extract Min from Empty Heap
const emptyHeap = new MinHeap();
test(emptyHeap.extractMin(), null, "Extract min from an empty heap");

// Edge Case: Extract Min from Single-Element Heap
const singleElementHeap = new MinHeap();
singleElementHeap.insert(99);
test(
  singleElementHeap.extractMin(),
  99,
  "Extract min from a single-element heap"
);
test(singleElementHeap.values, [], "Heap should be empty after extraction");

// Edge Case: Inserting Duplicate Values
const duplicateHeap = new MinHeap();
duplicateHeap.insert(50);
duplicateHeap.insert(50);
duplicateHeap.insert(50);
duplicateHeap.insert(50);
test(
  isMinHeap(duplicateHeap.values),
  true,
  "Heap should handle duplicate values correctly"
);

// Edge Case: Heapify a large number of elements
const largeHeap = new MinHeap();
const numbers = [5, 9, 20, 15, 30, 25, 60, 2, 50, 100, 40];
numbers.forEach((num) => largeHeap.insert(num));
test(largeHeap.extractMin(), 2, "Extract min from a large heap (should be 2)");
test(
  isMinHeap(largeHeap.values),
  true,
  "After extracting min, new root should be the second-smallest element (min-heap property)"
);

console.log("==== MinHeap Tests Completed ====");
